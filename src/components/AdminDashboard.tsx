...
import { BarChart3, Check, FileText, ImagePlus, LayoutDashboard, LogOut, Package, Pencil, Plus, Save, Settings, ShoppingBag, Truck, Users, Bot, X, Upload, Trash2, ListChecks, MessageCircle, ChevronUp, ChevronDown } from 'lucide-react';
...
export function AdminDashboard({ onExit }: AdminDashboardProps) {
  ...
  const [settings, setSettings] = useState({ oldUsername: '', oldPassword: '', newUsername: '', newPassword: '' });
  ...
  const signIn = async (event: FormEvent) => {
    ...
    const value = { username: login.username, password: login.password };
  ...
  if (!creds) return <AdminLogin login={login} setLogin={setLogin} error={loginError} onSubmit={signIn} onExit={onExit} />;
  ...
  return <div className="admin-shell" dir="rtl">
    ...
    <main className="admin-main"><header className="admin-topbar"><div><span>لوحة التحكم</span><h1>{nav.find((item) => item.id === tab)?.label}</h1></div><div className="admin-user"><span className="admin-avatar">A</span><span>{creds.username}</span></div></header>{notice && <div className="admin-notice"><Check size={17} /> {notice}<button onClick={() => setNotice('')}><X size={15} /></button></div>}
      ...
      {tab === 'settings' && <SettingsManager settings={settings} setSettings={setSettings} onNotice={setNotice} creds={creds} />}
...
function AdminLogin({ login, setLogin, error, onSubmit, onExit }: { login: { username: string; password: string }; setLogin: (value: { username: string; password: string }) => void; error: string; onSubmit: (event: FormEvent) => void; onExit: () => void }) {
  return <div className="admin-login" dir="rtl"><div className="admin-login-card"><button className="admin-back" onClick={onExit}>العودة للموقع</button><img src="/logo.jpeg" alt="Al-Mokhtar" /><div className="section-label">CONTROL CENTER</div><h1>لوحة الإدارة</h1><p>سجّل دخولك لإدارة كل محتوى الموقع.</p><form onSubmit={onSubmit}><label>اسم المستخدم<input value={login.username} onChange={(e) => setLogin({ ...login, username: e.target.value })} required /></label><label>كلمة المرور<input type="password" value={login.password} onChange={(e) => setLogin({ ...login, password: e.target.value })} required /></label>{error && <div className="admin-error">{error}</div>}<button className="admin-primary" type="submit">دخول لوحة التحكم</button></form></div></div>;
}
...
function SettingsManager({ settings, setSettings, onNotice, creds }: { settings: { oldUsername: string; oldPassword: string; newUsername: string; newPassword: string }; setSettings: (settings: { oldUsername: string; oldPassword: string; newUsername: string; newPassword: string }) => void; onNotice: (value: string) => void; creds: { username: string; password: string } }) {
  const save = async (event: FormEvent) => {
    event.preventDefault();
    const { data, error } = await supabase.rpc('admin_change_credentials', {
      p_username: settings.oldUsername,
      p_password: settings.oldPassword,
      p_new_username: settings.newUsername,
      p_new_password: settings.newPassword,
    });
    if (error || !data) { onNotice('البيانات القديمة غير صحيحة'); return; }
    onNotice('تم تغيير بيانات الدخول');
    setSettings({ oldUsername: '', oldPassword: '', newUsername: '', newPassword: '' });
  };
  return <div className="admin-view"><div className="admin-section-intro"><h2>إعدادات لوحة التحكم</h2><p>غيّر اسم المستخدم وكلمة المرور الخاصة بالدخول.</p></div><form className="settings-card" onSubmit={save}><h3>تغيير بيانات الدخول</h3><label>اسم المستخدم الحالي<input value={settings.oldUsername} onChange={(e) => setSettings({ ...settings, oldUsername: e.target.value })} required /></label><label>كلمة المرور الحالية<input type="password" value={settings.oldPassword} onChange={(e) => setSettings({ ...settings, oldPassword: e.target.value })} required /></label><hr /><label>اسم المستخدم الجديد<input value={settings.newUsername} onChange={(e) => setSettings({ ...settings, newUsername: e.target.value })} required /></label><label>كلمة المرور الجديدة<input type="password" minLength={6} value={settings.newPassword} onChange={(e) => setSettings({ ...settings, newPassword: e.target.value })} required /></label><button className="admin-primary" type="submit"><Save size={17} /> حفظ بيانات الدخول</button></form></div>;
}
